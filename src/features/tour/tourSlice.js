...
export const searchTours = createAsyncThunk("tour/searchTours", async (search, { rejectWithValue }) => {
  try {
    const response = await api.searchTours(search);
    return response.data;
  } catch (err) {
    return rejectWithValue(err.response.data.message);
  }
...
export const { setCurrentPage, numberOfPages } = tourSlice.actions;
